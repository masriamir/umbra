...
**Type annotations:** All Python code must carry complete type annotations — `ty` enforces strict type checking (`unresolved-attribute` errors are suppressed only for Django ORM internals; all other rules are active). Use modern union syntax (`X | None`, `list[str]`, `dict[str, int]`) rather than `Optional`, `List`, `Dict` from `typing`. Never leave a function untyped or return `Any` without explicit justification.
